...
async function recomputeScoreboard(
  supabase: any,
  matchId: string,
  match: { team1_id: string; team2_id: string; overs_per_innings: number | null },
) {
  ...
  const { data: allBalls, error: ballsError } = await supabase
    .from("balls")
...
export async function PUT(request: Request, context: RouteContext) {
  const { matchId, ballId } = context.params

  if (!matchId || !ballId) {
    return NextResponse.json({ error: "Match id and ball id are required" }, { status: 400 })
  }
  ...
  const { data: existingBall, error: existingBallError } = await supabase
    .from("balls")
    .select("id, innings, over_number, ball_number")
    .eq("id", ballId)
    .eq("match_id", matchId)
    .maybeSingle()

  if (existingBallError) {
    return NextResponse.json({ error: existingBallError.message }, { status: 500 })
  }
  ...
  if (existingBall.innings !== payload.innings) {
    return NextResponse.json({ error: "Cannot move a recorded delivery to another innings" }, { status: 400 })
  }
  ...
  try {
    await recomputeScoreboard(supabase, matchId, match)
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Failed to update scoreboard" }, { status: 500 })
  }

  return NextResponse.json({ data: { id: ballId } }, { status: 200 })
}
